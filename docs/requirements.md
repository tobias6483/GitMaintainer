...
| Dependency count summaries | Partial | Parses counts for `package.json`, `requirements.txt`, `composer.json`, `go.mod`, and `Cargo.toml`; other manifests are reported without dependency parsing. |

## Planned Improvements

- Add dependency-file parsing for more package ecosystems beyond npm, pip, Composer, Go modules, and Cargo.
